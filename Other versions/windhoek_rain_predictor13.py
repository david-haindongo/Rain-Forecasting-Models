...
Version: 12.8 (Fixed convergence warnings and probability display)
License: MIT
"""
...
# Specifically suppress the convergence warning
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('windhoek_rain_predictor_v12.log'),
...
class WindhoekRainPredictorV12:
    ...
    def run(self) -> None:
        ...
        if df.empty or len(df) < 100:  # Ensure enough data for training
        ...
        while True:
            try:
                ...
                # For custom datetime, we need to calculate the forecast hours
                if forecast_hours == 'custom':
                    target_datetime = self.get_custom_datetime()
                    # Calculate hours from now to target datetime
                    now = datetime.now()
                ...
                # Check if we have both classes after creating target
                ...
                if not self.ask_another():
                    print("\n" + "="*80)
                    print("Thank you for using Windhoek Rain Predictor v12.")
                    print("="*80)
                    break
    ...
    def get_location_config(self) -> None:
        """Configure location with custom lat/lon or use default Windhoek."""
    ...
    def get_model_selection(self) -> None:
        """Allow user to select which base models to use."""
        print("\n  MODEL SELECTION")
        print("  " + "-" * 30)
    ...
    def print_header(self) -> None:
        ...
        print("\n" + "=" * 80)
        print("WINDHOEK RAIN PREDICTOR v12.8".center(80))
        print("=" * 80)
    ...
    def get_user_timeframe(self) -> Union[int, str]:
        """Interactively select prediction horizon with custom option."""
    ...
    def get_custom_datetime(self) -> datetime:
        """Get custom datetime from user."""
        while True:
            try:
                date_str = input("\n  Enter target date and time (dd/mm/yyyy hh:mm): ").strip()
                target_dt = datetime.strptime(date_str, "%d/%m/%Y %H:%M")
                
                # Validate not too far in the future (max 7 days for reasonable prediction)
                now = datetime.now()
    ...
    # ------------------------------------------------------------------------
    # Data acquisition - Uses data from 62 days ago to present
    # ------------------------------------------------------------------------
    def fetch_from_openmeteo(self) -> Optional[Dict]:
        """Fetch data from Open-Meteo API."""
        url = "https://api.open-meteo.com/v1/forecast"
        
        # Use exactly 62 days of historical data
        ...
        params = {
            ...
            "past_days": past_days, 
            "forecast_days": 7,  # Need forecast data for future predictions
            "timezone": "auto"
        }
    ...
    def ingest_data(self) -> Tuple[pd.DataFrame, Dict]:
        ...
        if json_data and 'hourly' in json_data:
            ...
            # Filter to include data from start_date to present
            now = datetime.now()
    ...
    def _create_enhanced_synthetic_data(self) -> pd.DataFrame:
        """Create synthetic data for the required period."""
        print("  Generating synthetic dataset for the required period...")
        
        # Generate data from start_date to now + 7 days (for forecasts)
    ...
    def engineer_features_expert(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # PCA - but only on numeric columns
    ...
    def create_target_variable(self, df: pd.DataFrame, forecast_hours: int) -> pd.DataFrame:
        """Create binary target for rain in next forecast_hours."""
        print(f"\nCreating target for {forecast_hours} hours...")
        target_col = f'rain_next_{forecast_hours}h'
        rain_threshold = 0.2  # mm
        
        # Shift the precipitation forward to create target
        ...
        # Drop rows where target couldn't be calculated (end of series)
    ...
    def create_target_variable_alternative(self, df: pd.DataFrame, forecast_hours: int) -> pd.DataFrame:
        """Alternative target creation with different threshold if needed."""
    ...
    # ------------------------------------------------------------------------
    # Optimized training with user-selected models
    # ------------------------------------------------------------------------
    def train_optimized_ensemble(self, df: pd.DataFrame, forecast_hours: int) -> Dict:
        ...
        # Get feature columns - exclude target, precipitation, and regime columns
        ...
        # Store tuned models - only for selected models
        tuned_models = {}
        ...
        for name, model in tuned_models.items():
            try:
                ...
                calibrator = LogisticRegression(C=1e6, solver='lbfgs', max_iter=2000)  # Increased max_iter
                ...
                val_probas[name] = model.predict_proba(X_val_scaled)[:, 1]
    ...
    # ------------------------------------------------------------------------
    # Individual model training methods (with increased max_iter for LR)
    # ------------------------------------------------------------------------
    ...
    def _train_logistic_regression(self, X_train, y_train, cv) -> LogisticRegression:
        """Train Logistic Regression with hyperparameter tuning and increased max_iter."""
        lr_params = {
            'C': [0.1, 1, 10, 100],
            'solver': ['lbfgs', 'liblinear'],
            'class_weight': ['balanced', None],
            'max_iter': [2000, 5000]  # Increased max_iter to prevent convergence warnings
        }
    ...
    def predict(self, df: pd.DataFrame, model_dict: Dict, forecast_hours: int) -> Dict:
        ...
        # Ensure we have the features needed
        ...
        # Display - FIXED: Show actual probability value, not rounded to 0.0%
        ...
        print(f"\n  Stacked Ensemble:")
        print(f"     Probability: {ensemble_proba*100:.1f}%")  # This will show 0.0% if it's truly 0, but that's correct
...
# ============================================================================
def main():
    logging.info("Starting Windhoek Rain Predictor v12.8")
    try:
