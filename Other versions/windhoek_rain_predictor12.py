...
class WindhoekExpertPredictorV12:
    ...
    def __init__(self):
        ...
        self.current = None
        self.historical_pressure = None
        self.historical_temperature = None
        self.historical_humidity = None
    ...
    def engineer_features_expert(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Store historical data for frontend
        last_168 = features.iloc[-168:] if len(features) >= 168 else features
        self.historical_timestamps = [ts.strftime('%Y-%m-%d %H:%M:%S') for ts in last_168.index]
        self.historical_temperature = last_168['temperature'].tolist() if 'temperature' in last_168.columns else []
        self.historical_humidity = last_168['humidity'].tolist() if 'humidity' in last_168.columns else []
        self.historical_pressure = last_168['pressure'].tolist() if 'pressure' in last_168.columns else []
    ...
    def apply_hmm(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Store regime data for frontend
            if len(df) >= 168:
                last_168 = df.iloc[-168:]
                self.historical_regimes = {
                    'regime_0': last_168['regime_0'].tolist(),
                    'regime_1': last_168['regime_1'].tolist(),
                    'regime_2': last_168['regime_2'].tolist(),
                    'volatile_prob': last_168['volatile_prob'].tolist()
                }
            else:
                self.historical_regimes = {
                    'regime_0': df['regime_0'].tolist(),
                    'regime_1': df['regime_1'].tolist(),
                    'regime_2': df['regime_2'].tolist(),
                    'volatile_prob': df['volatile_prob'].tolist()
                }
    ...
    def train_optimized_ensemble(self, df: pd.DataFrame, forecast_hours: int) -> Dict:
        ...
        rf_search = RandomizedSearchCV(rf, rf_params, n_iter=10, cv=tscv, scoring=f2_scorer, random_state=42, n_jobs=-1)
        ...
        xgb_search = RandomizedSearchCV(xgb_model, xgb_params, n_iter=10, cv=tscv, scoring=f2_scorer, random_state=42, n_jobs=-1)
        ...
        lgb_search = RandomizedSearchCV(lgb_model, lgb_params, n_iter=10, cv=tscv, scoring=f2_scorer, random_state=42, n_jobs=-1)
        ...
        lr_search = RandomizedSearchCV(lr, lr_params, n_iter=8, cv=tscv, scoring=f2_scorer, random_state=42, n_jobs=-1)
        ...
        svm_search = RandomizedSearchCV(svm, svm_params, n_iter=8, cv=tscv, scoring=f2_scorer, random_state=42, n_jobs=-1)
        ...
        gbt_search = RandomizedSearchCV(gbt, gbt_params, n_iter=10, cv=tscv, scoring=f2_scorer, random_state=42, n_jobs=-1)
        ...
        et_search = RandomizedSearchCV(et, et_params, n_iter=10, cv=tscv, scoring=f2_scorer, random_state=42, n_jobs=-1)
        ...
        mlp_search = RandomizedSearchCV(mlp, mlp_params, n_iter=8, cv=tscv, scoring=f2_scorer, random_state=42, n_jobs=-1)
        ...
        ensemble_metrics = {
            ...
            'f2': f2_ens, 
            'auc_roc': auc_ens,
            'confusion_matrix': cm_ens.tolist(),  # Convert to list for JSON serialization
            'conformal_quantile': float(q),  # Convert to float
            'optimal_threshold': float(ensemble_thresh)  # Convert to float
        }
        ...
        # Return enhanced model dictionary with test metrics and all necessary data for frontend
        return {
            ...
            'conformal_quantile': float(q),  # Convert to float for JSON
            'optimal_threshold': float(ensemble_thresh),  # Convert to float for JSON
            'test_metrics': {  # Add test metrics for frontend display
                ...
                'optimal_threshold': float(ensemble_thresh)
            }
        }
    ...
    def predict(self, df: pd.DataFrame, model_dict: Dict, forecast_hours: int) -> Dict:
        ...
        test_metrics = model_dict.get('test_metrics', {})  # Get test metrics if available
        ...
        for name, (model, calib) in calibrated_models.items():
            ...
            cal_probas_dict[name] = float(cal)  # Convert to float for JSON

        # Stack for meta-model
        X_meta = np.array([list(cal_probas_dict.values())])
        ensemble_proba = float(meta_model.predict_proba(X_meta)[0][1])  # Convert to float

        # Conformal interval
        lower = float(max(0, ensemble_proba - q))  # Convert to float
        upper = float(min(1, ensemble_proba + q))  # Convert to float
        interval_width = float(upper - lower)  # Convert to float
        ...
        # Calculate statistics from individual model predictions
        individual_probs = list(cal_probas_dict.values())
        mean_prob = np.mean(individual_probs) * 100
        std_dev = np.std(individual_probs) * 100
        skewness = float(stats.skew(individual_probs)) if len(individual_probs) > 2 else 0
        kurtosis = float(stats.kurtosis(individual_probs)) if len(individual_probs) > 3 else 0
        ci_width = interval_width * 100
        ...
        if 'volatile_prob' in df.columns:
            volatile = float(df['volatile_prob'].iloc[-1])  # Convert to float
            regime_probs = [float(df[f'regime_{i}'].iloc[-1]) for i in range(3)]  # Convert to float
            regime_desc = f"regime probs: [{regime_probs[0]:.2f} {regime_probs[1]:.2f} {regime_probs[2]:.2f}]"
        else:
            volatile = 0.5
            regime_probs = [0.33, 0.33, 0.34]
        ...
        try:
            ...
            if len(pressure) > 100:
                hurst = float(nolds.hurst_rs(pressure))  # Convert to float
        ...
        historical_data = {
            'timestamps': self.historical_timestamps if self.historical_timestamps else [],
            'temperature': self.historical_temperature if self.historical_temperature else [],
            'humidity': self.historical_humidity if self.historical_humidity else [],
            'pressure': self.historical_pressure if self.historical_pressure else [],
            'regime_0': self.historical_regimes['regime_0'] if self.historical_regimes else [],
            'regime_1': self.historical_regimes['regime_1'] if self.historical_regimes else [],
            'regime_2': self.historical_regimes['regime_2'] if self.historical_regimes else [],
            'volatile_prob': self.historical_regimes['volatile_prob'] if self.historical_regimes else []
        }

        # Enhanced return dictionary with all necessary data for frontend
        return {
            'timeframe': forecast_hours,
            'timeframe_name': self.timeframe_names[forecast_hours],
            'probability': float(ensemble_proba * 100),  # Convert to float
            'probability_lower': float(lower * 100),  # Convert to float
            'probability_upper': float(upper * 100),  # Convert to float
            'interval_width': float(interval_width * 100),  # Convert to float
            'outcome': outcome,
            'outcome_full': outcome_full,
            'confidence': conf_level,
            'trust_status': trust_status,
            'trust_reasons': trust_reasons,
            'individual_predictions': {name: float(proba * 100) for name, proba in cal_probas_dict.items()},  # Convert to float
            'model_thresholds': {name: float(thresh) for name, thresh in model_thresholds.items()},  # Convert to float
            'volatile_prob': float(volatile * 100),  # Convert to float
            'regime': regime_desc,
            'regime_probs': regime_probs,  # List of floats
            'hurst': hurst,  # Float or None
            'data_quality': float(self.data_quality_score * 100),  # Convert to float
            'test_metrics': test_metrics,  # Include test metrics from training
            'historical_data': historical_data,  # Add historical data for frontend charts
            'stats': {  # Add statistics for frontend summary
