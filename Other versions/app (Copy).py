...
Connects to windhoek_rain_predictor16.py for all ML functionality.
...
from windhoek_rain_predictor16 import WindhoekRainPredictorV12
...
def get_timeframes_list():
    ...
    temp_predictor = WindhoekRainPredictorV12()
    timeframes = [
        {
            'key': key,
...
def initialize():
    ...
    try:
        ...
        location = app.config['LOCATIONS'].get(location_id, app.config['DEFAULT_LOCATION'])
...
def switch_location():
    """Switch to a new location and fetch fresh data."""
    try:
        data = request.json or {}
        location_id = data.get('location', 'windhoek')
