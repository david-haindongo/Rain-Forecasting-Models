...
Version: 14.2 (Fixed NOAA real-time precipitation with METAR fallback)
License: MIT
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs_rain_predictor_v14.log'),
...
class DavidRainPredictor:
    ...
    Version 14.2 includes NOAA/METAR real-time precipitation data.
    """
    ...
    def __init__(self):
        ...
        # Known weather stations in Namibia for direct lookup
        """
        self.namibia_stations = [
            {'id': 'FYWH', 'name': 'Windhoek Hosea Kutako International Airport', 'lat': -22.4867, 'lon': 17.4708},
            {'id': 'FYWE', 'name': 'Windhoek Eros Airport', 'lat': -22.6122, 'lon': 17.0804},
            {'id': 'FYGF', 'name': 'Grootfontein Airport', 'lat': -19.6022, 'lon': 18.1227},
            {'id': 'FYKT', 'name': 'Keetmanshoop Airport', 'lat': -26.5397, 'lon': 18.1114},
            {'id': 'FYWB', 'name': 'Walvis Bay Airport', 'lat': -22.9799, 'lon': 14.6453},
            {'id': 'FYOA', 'name': 'Ondangwa Airport', 'lat': -17.8782, 'lon': 15.9528},
            {'id': 'FYRU', 'name': 'Rundu Airport', 'lat': -17.9565, 'lon': 19.7194},
            {'id': 'FYLZ', 'name': 'Lüderitz Airport', 'lat': -26.6874, 'lon': 15.2429},
        ]
        """
        # Replaces hardcoded namibia_stations with dynamic loading
        self.airport_db_path = os.path.join(self.cache_dir, "airports.csv")
        self.namibia_stations = self._load_airport_database()

    
    def _load_airport_database(self) -> List[Dict]:
        """
        Loads airport data and strictly filters for METAR-capable reporting stations.
        Prioritizes 4-letter ICAO codes and established airport types.
        """
        sources = [
        ...
        if os.path.exists(self.airport_db_path) and os.path.getsize(self.airport_db_path) > 0:
            print(f"  ✓ Loading airport database from {self.airport_db_path}")
            df_airports = pd.read_csv(self.airport_db_path)
        elif os.path.exists("airports.csv") and os.path.getsize("airports.csv") > 0:
            print("  ✓ Loading airport database from airports.csv")
            df_airports = pd.read_csv("airports.csv")
        else:
            print("  ! Airport database missing. Downloading global database...")
            for url in sources:
                try:
                    if url.endswith('.csv'):
                        ...
                        df_airports = pd.DataFrame(resp.json())
                    
                    if not df_airports.empty:
                        df_airports.to_csv(self.airport_db_path, index=False)
                        break
        ...
        if df_airports.empty:
            return [{'id': 'FYWH', 'name': 'Windhoek Hosea Kutako', 'lat': -22.4867, 'lon': 17.4708}]

        stations = []
        try:
            # Improved column mapping
            id_col = next((c for c in ['ident', 'icao', 'gps_code'] if c in df_airports.columns), None)
            lat_col = next((c for c in ['latitude_deg', 'lat', 'latitude'] if c in df_airports.columns), None)
            lon_col = next((c for c in ['longitude_deg', 'lon', 'longitude'] if c in df_airports.columns), None)
            name_col = next((c for c in ['name', 'airport_name'] if c in df_airports.columns), 'name')
            type_col = next((c for c in ['type', 'airport_type'] if c in df_airports.columns), None)

            for _, row in df_airports.iterrows():
                icao = str(row[id_col]).strip().upper()
                
                # CRITICAL FILTER: Only keep 4-character codes (ICAO standard)
                # This skips local IDs like 'AO-0037' (Calai) which don't support METAR
                if len(icao) != 4 or pd.isna(row[lat_col]) or pd.isna(row[lon_col]):
                    continue

                # OPTIONAL: Uncomment to prioritize only major airports
                # if type_col and row[type_col] not in ['medium_airport', 'large_airport']:
                #    continue
            ...
            print(f"  ✓ Successfully parsed {len(stations)} reporting stations (ICAO filtered)")
        except Exception as e:
        ...
        while True:
            try:
                ...
                if not self.ask_another():
                    print("\n" + "="*80)
                    print("Thank you for using David Rain Predictor v14.")
                    print("="*80)
                    break
    ...
    def print_header(self) -> None:
        print("\n" + "=" * 80)
        print("DAVID RAIN PREDICTOR v14.2".center(80))
        print("=" * 80)
        print(f"CURRENT TIME : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}".center(80))
        print("-" * 80)
        print("Rain Prediction/Forecasting System".center(80))
        print()
        ...
        print("• Model selection: Automatic exclusion of poor performers based on validation F2 score (< 0.5)")
        print("  (Threshold-independent – models are judged by actual performance, not arbitrary cutoff values)")
        print("• Manual threshold override: Ensures ensemble threshold stays in 0.2-0.4 range for optimal recall")
        print("• Real-time Data: NOAA/METAR integration for accurate current precipitation")
        print("-" * 80)
    ...
    def fetch_noaa_current_conditions(self) -> Optional[float]:
        """
        Fetch REAL-TIME weather observations from NOAA weather stations.
        Completely free, no API key required.
        Returns precipitation in mm if available.
        """
        try:
            # Step 1: Try NOAA points API
            points_url = f"https://api.weather.gov/points/{self.lat},{self.lon}"
            
            headers = {
                'User-Agent': 'DavidRainPredictor/1.0 (davidhaindongoh@gmail.com)',
                'Accept': 'application/json'
            }
            
            print("  Fetching real-time precipitation from weather stations...")
            response = requests.get(points_url, headers=headers, timeout=10)
            
            # If points endpoint fails (404 for non-US locations), try direct station lookup
            if response.status_code == 404:
                print("  NOAA points API not available for this region, trying METAR stations...")
                return self._fetch_from_metar()
                
            response.raise_for_status()
            points_data = response.json()
            
            if 'properties' not in points_data or 'observationStations' not in points_data['properties']:
                print("  No station data available from NOAA")
                return self._fetch_from_metar()
            ...
            if not stations_data.get('features'):
                print("  No NOAA weather stations found nearby")
                return self._fetch_from_metar()
            
            # Get the nearest station
            nearest_station = stations_data['features'][0]['id']
            station_name = stations_data['features'][0]['properties'].get('name', 'Unknown station')
            print(f"    Found NOAA station: {station_name}")
            
            # Get latest observations
            obs_url = f"{nearest_station}/observations/latest"
            obs_response = requests.get(obs_url, headers=headers, timeout=10)
            obs_response.raise_for_status()
            obs_data = obs_response.json()
            
            return self._extract_precipitation_from_observation(obs_data)
            
        except requests.exceptions.RequestException as e:
            logger.warning(f"NOAA API request failed: {e}")
            return self._fetch_from_metar()
        except Exception as e:
            logger.warning(f"NOAA API parsing failed: {e}")
            return self._fetch_from_metar()


    def _fetch_from_metar(self) -> Optional[float]:
        """
        Fetch real-time precipitation with a multi-station fallback loop.
        Ensures that if the closest station is silent, it checks the next nearest.
        """
        try:
            if not self.namibia_stations:
                return None

            # 1. Sort ALL reporting stations by distance from the user's input coordinates
            stations_by_dist = sorted(
                self.namibia_stations, 
                key=lambda x: ((x['lat'] - self.lat)**2 + (x['lon'] - self.lon)**2)**0.5
            )
            
            headers = {
                'User-Agent': 'DavidRainPredictor/1.1 (davidhaindongoh@gmail.com)',
                'Accept': 'application/json'
            }

            # 2. Try the nearest 5 stations in order
            for station in stations_by_dist[:5]:
                icao = station['id']
                
                # Try AviationWeather.gov API
                metar_url = f"https://aviationweather.gov/api/data/metar?ids={icao}&format=json"
                
                try:
                    response = requests.get(metar_url, headers=headers, timeout=5)
                    if response.status_code == 200:
                        metar_data = response.json()
                        if metar_data and len(metar_data) > 0:
                            metar_text = metar_data[0].get('rawOb', '')
                            print(f"    ✓ Using data from station: {station['name']} ({icao})")
                            return self._extract_precipitation_from_metar(metar_text)
                    
                    # Secondary Fallback for this specific station: NOAA API
                    station_url = f"https://api.weather.gov/stations/{icao}/observations/latest"
                    response = requests.get(station_url, headers=headers, timeout=5)
                    if response.status_code == 200:
                        obs_data = response.json()
                        print(f"    ✓ Using data from NOAA station: {station['name']} ({icao})")
                        return self._extract_precipitation_from_observation(obs_data)
                        
                except Exception:
                    # If this station fails/times out, the loop continues to the next closest
                    continue
            
            print("  ⚠ No active METAR reporting stations responded within range.")
            return None
            
        except Exception as e:
            logger.warning(f"METAR fetch loop failed: {e}")
            return None

    def _extract_precipitation_from_observation(self, obs_data: Dict) -> float:
        """Extract precipitation from NOAA observation data."""
        try:
            precipitation = 0.0
            props = obs_data.get('properties', {})
            
            # Check for precipitation in the last hour
            if 'precipitationLastHour' in props and props['precipitationLastHour'].get('value') is not None:
                # Value is in meters, convert to mm
                precipitation = props['precipitationLastHour']['value'] * 1000
                print(f"    Last hour precipitation: {precipitation:.1f}mm")
            elif 'precipitationLast3Hours' in props and props['precipitationLast3Hours'].get('value') is not None:
                # Value is in meters, convert to mm and average per hour
                precipitation = (props['precipitationLast3Hours']['value'] * 1000) / 3
                print(f"    Last 3 hours average: {precipitation:.1f}mm")
            elif 'precipitationLast6Hours' in props and props['precipitationLast6Hours'].get('value') is not None:
                # Value is in meters, convert to mm and average per hour
                precipitation = (props['precipitationLast6Hours']['value'] * 1000) / 6
                print(f"    Last 6 hours average: {precipitation:.1f}mm")
            
            # Also check if it's currently raining (present weather codes)
            raw_message = props.get('rawMessage', '')
            text_desc = props.get('textDescription', '').lower()
            
            if 'RA' in raw_message or 'DZ' in raw_message or 'SH' in raw_message or 'rain' in text_desc:
                if precipitation == 0:
                    precipitation = 0.5
                    print("    Light rain detected from weather codes")
            
            return precipitation
            
        except Exception as e:
            logger.warning(f"Failed to parse observation data: {e}")
            return None

    def _extract_precipitation_from_metar(self, metar_text: str) -> float:
        """Extract precipitation from METAR text."""
        try:
            precipitation = 0.0
            
            # Check for rain indicators in METAR
            rain_indicators = ['RA', 'DZ', 'SHRA', 'TSRA', '+RA', '-RA', 'RERA']
            has_rain = any(indicator in metar_text for indicator in rain_indicators)
            
            if has_rain:
                # Look for precipitation amount (usually in the RMK section)
                # Common patterns: P0000 (hundredths of inches), 60000 (6-hour precip)
                
                # Try P#### format (hundredths of inches)
                p_match = re.search(r'P(\d{4})', metar_text)
                if p_match:
                    precip_hundredths = int(p_match.group(1))
                    precipitation = precip_hundredths * 0.254  # Convert to mm
                    print(f"    METAR precipitation (P-code): {precipitation:.1f}mm")
                else:
                    # Try 6#### format (6-hour precip in hundredths of inches)
                    six_match = re.search(r'6(\d{4})', metar_text)
                    if six_match:
                        precip_hundredths = int(six_match.group(1))
                        precipitation = precip_hundredths * 0.254  # Convert to mm
                        print(f"    METAR 6-hour precipitation: {precipitation:.1f}mm")
                    else:
                        # If no amount but rain reported, assume light rain
                        precipitation = 0.5
                        print(f"    METAR indicates rain (no amount): {precipitation:.1f}mm")
            
            return precipitation
            
        except Exception as e:
            logger.warning(f"Failed to parse METAR: {e}")
            return 0.0
    ...
    def handle_missing_data(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, float]:
        ...
        df.to_csv('RAW_historical_data_v14.csv')
        try:
            df.to_excel('RAW_historical_data_v14.xlsx', index=True)
        except ImportError:
    ...
    def ingest_data(self) -> Tuple[pd.DataFrame, Dict]:
        """
        Enhanced data ingestion with NOAA/METAR real-time precipitation data.
        Open-Meteo provides historical data, METAR provides REAL current conditions.
        """
        print("\nSTEP 1: Data Ingestion")
        print("-"*40)
        print(f"  Using data from the last {self.days_lookback} days ({self.start_date.strftime('%Y-%m-%d')} to present)")
        
        # Get historical data from Open-Meteo (good for trends, but model-based)
        ...
        if json_data and 'hourly' in json_data:
            ...
            # Get Open-Meteo current data (model-based)
            current = json_data.get('current', {})
            current_precip_model = current.get('precipitation', 0)
            
            # OVERRIDE: Get REAL-TIME precipitation from weather stations
            realtime_precip = self.fetch_noaa_current_conditions()
            
            if realtime_precip is not None:
                # Override the current precipitation with REAL observed data
                current['precipitation'] = realtime_precip
                current['precipitation_source'] = 'METAR/NOAA (real-time station)'
                precip_status = "RAINING" if realtime_precip > 0.1 else "NOT RAINING"
                print(f"  ✓ Real-time station reports {realtime_precip:.1f}mm precipitation")
            else:
                # Fallback to Open-Meteo model data
                current['precipitation_source'] = 'Open-Meteo (model)'
                precip_status = "RAINING" if current_precip_model > 0.1 else "NOT RAINING"
                print(f"  ⚠ Using Open-Meteo model data (real-time station unavailable)")
            
            print(f"  Live Data: Atmospheric Condition [{self.location_name}]")
            ...
            print(f"     Precipitation: {current.get('precipitation', 0):.1f}mm ({precip_status})")
            print(f"     Source: {current.get('precipitation_source', 'Open-Meteo')}")
            ...
            # Save to files
            df.to_csv('weather_historical_data_v14.csv')
            try:
                df.to_excel('weather_historical_data_v14.xlsx', index=True)
            except ImportError:
                print("  Note: openpyxl not installed. Skipping Excel export.")
                print("  Install with: pip install openpyxl")
    ...
    def _create_enhanced_synthetic_data(self) -> pd.DataFrame:
        """Generate synthetic data when APIs are unavailable."""
        ...
        # Convert Index objects to numpy arrays to avoid attribute errors
        hour_of_day = np.array(dates.hour)
        ...
        # Compute pressure difference (hourly change) and pad to keep same length
        pressure_diff = np.abs(np.diff(pressure)) > 4
        pressure_diff_padded = np.concatenate([[False], pressure_diff])  # first hour has no previous value
        
        prob_rain = (0.03 + 
                     0.25 * (humidity > 75) + 
                     0.2 * (pressure < 845) + 
                     0.1 * pressure_diff_padded)
    ...
    # ------------------------------------------------------------------------
    # Helper: balance validation set
    # ------------------------------------------------------------------------
    def _balance_validation_set(self, X_val, y_val, target_ratio):
        """Subsample validation set to match training class distribution."""
    ...
    # ------------------------------------------------------------------------
    # Optimized training with model selection and threshold override
    # ------------------------------------------------------------------------
    def train_optimized_ensemble(self, df: pd.DataFrame, forecast_hours: int) -> Dict:
        ...
        # --------------------------------------------------------------------
        # Find per-model optimal thresholds
        # --------------------------------------------------------------------
        model_thresholds = {}
        model_val_scores = {}
        
        # Split validation set into 3 chronological periods for stability checking
        val_size = len(y_val_balanced)
        period_size = max(1, val_size // 3)  # Ensure at least 1 sample per period
        
        # Create three consecutive periods
        val_periods = [
            (0, period_size),
            (period_size, min(2 * period_size, val_size)),
            (min(2 * period_size, val_size), val_size)
        ]
        
        # Adjust last period if needed
        if val_periods[2][0] >= val_periods[2][1]:
            # Last period is empty, merge with middle
            val_periods = [
                (0, period_size),
                (period_size, val_size)
            ]
        
        print(f"\n  Evaluating models across {len(val_periods)} validation periods...")
        
        for name, (model, calib) in calibrated_models.items():
            cal_val = val_probas[name]
            
            if val_has_both:
                # Calculate threshold on full validation set (for threshold selection)
                thresh, full_f2 = find_optimal_threshold(y_val_balanced, cal_val, metric='f2', beta=2)
                
                # Calculate F2 on each validation period using the SAME threshold
                period_scores = []
                period_details = []
                
                for period_idx, (start_idx, end_idx) in enumerate(val_periods):
                    y_period = y_val_balanced.iloc[start_idx:end_idx]
                    cal_period = cal_val[start_idx:end_idx]
                    
                    # Skip if period has only one class
                    if len(np.unique(y_period)) < 2:
                        period_details.append(f"P{period_idx+1}: single-class")
                        continue
                    
                    # Calculate F2 for this period using the threshold
                    pred_period = (cal_period >= thresh).astype(int)
                    f2_period = fbeta_score(y_period, pred_period, beta=2, zero_division=0)
                    period_scores.append(f2_period)
                    period_details.append(f"P{period_idx+1}: {f2_period:.3f}")
                
                # Calculate statistics
                if period_scores:
                    avg_period_f2 = np.mean(period_scores)
                    std_period_f2 = np.std(period_scores)
                    min_period_f2 = np.min(period_scores)
                    max_period_f2 = np.max(period_scores)
                    
                    period_summary = " | ".join(period_details)
                    print(f"    {name}: {period_summary}")
                    print(f"      → Period F2: min={min_period_f2:.3f}, avg={avg_period_f2:.3f}±{std_period_f2:.3f}, max={max_period_f2:.3f}")
                    
                    # Store the MINIMUM period F2 as the model's score (conservative approach)
                    # This ensures model performs well across all regimes
                    model_val_scores[name] = min_period_f2
                    
                    # Check if any period has poor performance
                    poor_periods = [f"P{i+1}" for i, score in enumerate(period_scores) if score < 0.5]
                    if poor_periods:
                        print(f"      ⚠ Poor performance in: {', '.join(poor_periods)}")
                else:
                    # No valid periods with both classes
                    model_val_scores[name] = full_f2
                    print(f"    {name}: No multi-class periods, using full validation F2={full_f2:.3f}")
            else:
                # Validation single-class case
                thresh = 0.5
                pred = (cal_val >= thresh).astype(int)
                full_f2 = fbeta_score(y_val_balanced, pred, beta=2, zero_division=0)
        ...
        # --------------------------------------------------------------------
        # Model selection based on multi-period stability
        # --------------------------------------------------------------------
        good_models = {}
        
        if val_has_both:
            for name in calibrated_models.keys():
                f2_val = model_val_scores[name]  # This is now the MIN F2 across periods
                thresh = model_thresholds[name]
                
                # Primary criterion: F2 score must be at least 0.5 in ALL periods
                if f2_val < 0.5:
                    print(f"  ⚠ Excluding {name}: minimum period F2={f2_val:.3f} (<0.5) - inconsistent across validation periods")
                else:
                    # Keep the model - it performed well in all periods
                    good_models[name] = calibrated_models[name]
                    self.model_thresholds[name] = thresh
                    self.model_validation_scores[name] = f2_val
                    
                    # Log threshold information with context
                    if thresh < 0.05:
                        print(f"  ✓ Keeping {name}: min F2={f2_val:.3f} (threshold={thresh:.3f} - low but stable across periods)")
                    elif thresh > 0.7:
                        print(f"  ✓ Keeping {name}: min F2={f2_val:.3f} (threshold={thresh:.3f} - high but stable across periods)")
                    else:
                        print(f"  ✓ Keeping {name}: min F2={f2_val:.3f} (threshold={thresh:.3f})")
        else:
            # Validation single-class - keep all models with default threshold 0.5
        ...
        # Fallback if all models were excluded
        ...
        print(f"\n  Using {len(good_models)} stable models for ensemble: {list(good_models.keys())}")
        ...
        # Ensure threshold isn't too conservative (which hurts recall)
        if ensemble_thresh > 0.4:
            print(f"  ⚠ Auto-tuned threshold {ensemble_thresh:.2f} is too conservative. Lowering to 0.35 to improve recall")
            ensemble_thresh = 0.35
        elif ensemble_thresh < 0.2:
            print(f"  ⚠ Auto-tuned threshold {ensemble_thresh:.2f} is too low. Raising to 0.25 to reduce false alarms")
            ensemble_thresh = 0.25
        # =====================================
    ...
    def _train_logistic_regression(self, X_train, y_train, cv) -> LogisticRegression:
        # We increase max_iter and include the 'saga' solver which handles 
        # many features and large datasets better than lbfgs.
        lr_params = {
            'C': [0.1, 1, 10, 100],
            'solver': ['liblinear', 'saga'], # saga is robust for high-dimensional data
            'class_weight': ['balanced'],    # Keep balanced to handle rain/no-rain imbalance
            'max_iter': [5000, 10000],       # Significant increase to ensure convergence
            'tol': [1e-4, 1e-3]              # Tolerance for stopping criteria
        }
        
        lr = LogisticRegression(random_state=42)
        
        # Increase n_iter to ensure we try the saga/high-iter combinations
    ...
    def predict(self, df: pd.DataFrame, model_dict: Dict, forecast_hours: int) -> Dict:
        ...
        for name, proba in cal_probas_dict.items():
            thresh = model_thresholds[name]
            model_outcome = "RAIN" if proba >= thresh else "NO RAIN"
    ...
    def save_results(self, prediction: Dict, current: Dict, target_datetime: Optional[datetime] = None) -> None:
        ...
        record = {
            ...
            'precipitation_source': current.get('precipitation_source', 'Open-Meteo')
        }
...
# ============================================================================
def main():
    logging.info("David Rain Predictor Model v14.2")
    try:
